...
sealed class LoginStateEvent : State(){
    data class RequestTokenFailed(val statusCode : Int, val statusMessage : String) : LoginStateEvent()
    data class TimeOutError(val message : String) : LoginStateEvent()
    data class LoginFailed(val code : Int , val message: String ,val requestToken : String) : LoginStateEvent()
    data class SessionFailed(val message: String) : LoginStateEvent()
    data class Success(val session: String) : LoginStateEvent()
}
